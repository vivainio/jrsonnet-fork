...
[NOTE]
====
This is a personal fork of https://github.com/CertainLach/jrsonnet[CertainLach/jrsonnet].

Difference from upstream: `--preserve-order` is exposed as an always-available, global CLI flag (in upstream it lives behind the `exp-preserve-order` compile-time feature). When set, object field insertion order is preserved through `std.manifest*`, `std.objectFields*`, and object composition.
...
Releases from this fork are tagged `vX.Y.Z-fork.N`.
====
